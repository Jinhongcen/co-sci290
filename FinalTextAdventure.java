...
public class FinalTextAdventure{
  ...
  public static void main(String[] args){
    ...
    int k =0; //use to count when you win 5 time you win the game 
    //story setup
    String b=" ";
    ...
    while(m==0){
      int mynum = 1 + (int)(Math.random()*(3-1));
      ...
      if(younum == mynum){
        ...
        mynum = 1 + (int)(Math.random()*(3-1));
        
        younum = tool.Ifwrongnumber(younum);
      
        if(k==6){// when win 6 time , will come here
          m=k;
          tool.readFile("testWriting.txt");//read stroy line
